...
class SharedConfigs(object):
    ...
    def __init__(self, desc="shared config for pretraining and finetuning"):
        ...
        parser.add_argument("--data_ratio", type=float, default=1.0,
                            help="portion of train/val exampels to use, e.g., overfit a small set of data")

        # ========== Required parameters ==========
        parser.add_argument("--model_config", type=str, help="path to model structure config json")
        parser.add_argument("--tokenizer_dir", type=str, help="path to tokenizer dir")
        parser.add_argument("--output_dir", type=str, help="dir to store model checkpoints & training meta.")

        # ========== Data preprocessing parameters ==========
        parser.add_argument("--max_txt_len", type=int, default=20, help="max text #tokens ")
        parser.add_argument("--img_pixel_mean", type=float, default=None, nargs=3, help="image pixel mean")
        parser.add_argument("--img_pixel_std", type=float, default=None, nargs=3, help="image pixel std")
        parser.add_argument("--img_input_format", type=str, default="BGR", choices=["BGR", "RGB"],
                            help="image input format is BGR for detectron2")
        parser.add_argument("--max_n_example_per_group", type=int, default=1,
                            help="max #examples (e.g., captions) paired with each image/video in an input group."
                                 ...
                                 "to reduce image processing time, including basic transform (resize, etc) and CNN "
                                 "encoding")
        ...
        parser.add_argument("--score_agg_func", type=str, default="mean", choices=["mean", "max", "lse"],
        ...
        parser.add_argument("--num_valid", default=20, type=int,
                            help="Run validation X times during training and checkpoint.")
        parser.add_argument("--min_valid_steps", default=100, type=int,
                            help="minimum #steps between two validation runs")
        parser.add_argument("--save_steps_ratio", default=0.01, type=float,
                            help="save every 0.01*global steps to resume after preemption,not used for checkpointing.")
        parser.add_argument("--num_train_epochs", default=10, type=int,
                            help="Total #training epochs.")
        parser.add_argument("--optim", default="adamw", choices=["adam", "adamax", "adamw"],
                            help="optimizer")
        parser.add_argument("--betas", default=[0.9, 0.98], nargs=2, help="beta for adam optimizer")
        parser.add_argument("--decay", default="linear", choices=["linear", "invsqrt"],
                            help="learning rate decay method")
        parser.add_argument("--dropout", default=0.1, type=float, help="tune dropout regularization")
        parser.add_argument("--weight_decay", default=1e-3, type=float, help="weight decay (L2) regularization")
        parser.add_argument("--grad_norm", default=2.0, type=float, help="gradient clipping (-1 for no clipping)")
        ...
        parser.add_argument("--step_decay_epochs", type=int, nargs="+", help="multi_step decay epochs")

        # ========== Model arch ==========
        parser.add_argument("--model_type", type=str, default="pretrain",
                            help="type of e2e model to use. Support only 'pretrain' for now. ")
        parser.add_argument("--timesformer_model_cfg", type=str, default="",
                            help="path to timesformer model cfg yaml")
        # ========== Checkpoint ==========
        parser.add_argument("--e2e_weights_path", type=str, help="path to e2e model weights")
        parser.add_argument("--clip_init", default=0, type=int, choices=[0, 1],
                            help="1 for using clip ckpt for init.")
        parser.add_argument("--bert_weights_path", type=str, help="path to BERT weights, only use for pretraining")

        # ========== Inference parameters ==========
        parser.add_argument("--inference_model_step", default=-1, type=str,
                            help="pretrained model checkpoint step")
        parser.add_argument("--do_inference", default=0, type=int, choices=[0, 1],
                            help="perform inference run. 0: disable, 1 enable")
        parser.add_argument("--inference_split", default="val",
                            help="For val, the data should have ground-truth associated it. For test*, the data comes "
                                 "with no ground-truth.")
        parser.add_argument("--inference_txt_db", type=str, help="path to txt_db file for inference")
        parser.add_argument("--inference_img_db", type=str, help="path to img_db file for inference")
        parser.add_argument("--inference_batch_size", type=int, default=64, help="single-GPU batch size for inference")
        ...
        parser.add_argument("--fp16", type=int, choices=[0, 1], default=0,
                            help="Use 16-bit float precision instead of 32-bit. 0: disable, 1 enable")
        parser.add_argument("--n_workers", type=int, default=4, help="#workers for data loading")
        parser.add_argument("--pin_mem", type=int, choices=[0, 1], default=1, help="pin memory. 0: disable, 1 enable")
    ...
    def parse_args(self):
        ...
        zero_one_options = ["fp16", "pin_mem", "use_mcl", "use_mlm", "use_itc", "debug", "do_inference"]
    ...
    def get_sparse_pretraining_args(self):
        # ========== Pre-training args ==========
        self.parser.add_argument("--use_mlm", type=int, choices=[0, 1], default=0,
                                 help="enable mlm loss. 0: disable, 1 enable")
        self.parser.add_argument("--use_itc", type=int, choices=[0, 1], default=0,
                                 help="enable itc loss. 0: disable, 1 enable")
        self.parser.add_argument("--use_mcl", type=int, choices=[0, 1], default=0,
                                 help="enable mcl loss. 0: disable, 1 enable")

        # sparse pretraining-specific settings
        self.parser.add_argument("--crop_img_size", type=int, default=256,
                                 help="crop size during pre-training.")
        self.parser.add_argument("--resize_size", type=int, default=288,
    ...
    def get_vqa_args(self):
        self.parser.add_argument("--ans2label_path", type=str, help="path to {answer: label} file")
        self.parser.add_argument("--loss_type", type=str, default="bce", help="loss type")
        self.parser.add_argument("--classifier", type=str, default="mlp", choices=["mlp", "linear"],
                                 help="classifier type")
        self.parser.add_argument("--cls_hidden_scale", type=int, default=2,
    ...
    def get_video_qa_args(self):
        self.parser.add_argument("--task", type=str,
                                 choices=["msrvtt_qa", "next_qa", "next_qa_oe"],
                                 help="QA datasets")
        ...
        # for next_qa
        self.parser.add_argument("--map_vid_vidorID_path", type=str, default=None,
                                 help="path to {answer: label} file")
        ...
        if args.task != 'next_qa_oe':
            if os.path.exists(args.ans2label_path):
                num_answers = len(json.load(open(args.ans2label_path, "r")))
            else:
                num_answers = 0
        elif args.task == 'next_qa_oe':
            if os.path.exists(args.ans2label_path):
                with open(args.ans2label_path, 'rb') as f:
                    vocab = pickle.load(f)
                    num_answers = len(vocab.word2idx)
        # else:
        #     if os.path.exists(args.map_vid_vidorID_path) and os.path.exists(args.map_vid_vidorID_path):
        #

        if args.task in ["msrvtt_qa", "msvd_qa", "next_qa_oe"]:
