...
def select_batch_text_pivots(inputs, tokenizer, ent2id, mpm_probability=1.0, is_train=True):
...
class VideoRandomSquareCrop(object):
    def __init__(self, crop_size, crop_type='consistent', p=0.5, ):
    ...
    def __call__(self, video):
        ...
        if isinstance(video, torch.Tensor):
            ...
            if self.crop_type == 'consistent':
                ...
                v = torch.zeros((video.shape[0], video.shape[1], self.crop_size, self.crop_size))
                for i in range(t):
                    x = random.randint(0, h - self.crop_size)
                    y = random.randint(0, w - self.crop_size)
                    v[:, i] = video[:, i, x: x + self.crop_size, y: y + self.crop_size]
                return v
            else:
                raise RuntimeError(f'The crop_type:{self.crop_type} is illegal.')
...
def repeat_tensor_rows(raw_tensor, row_repeats):
    ...
    Args:
        raw_tensor: (B, *) [5, 5, 5, 5, 5]
        row_repeats: list(int), len(row_repeats) == len(raw_tensor)
    """
    assert len(raw_tensor) == len(raw_tensor), "Has to be the same length"
    if sum(row_repeats) == len(row_repeats):    # sum([1, 1, 1, 1]) = len([1, 1, 1, 1])
        return raw_tensor
    else:
        indices = torch.LongTensor(
            flat_list_of_lists([[i] * r for i, r in enumerate(row_repeats)])    # for i, r in enumerate([5, 5, 5, 5, 5])
