    ...
    def __iter__(self):
        ...
        while batch is not None:
            ...
            if self.img_normalize is not None:
                ...
                if "rdm_crop_inputs" in batch:
                    batch["rdm_crop_inputs"] = batch["rdm_crop_inputs"].float()
                    batch["rdm_crop_inputs"] = self.img_normalize(batch["rdm_crop_inputs"])
                if "tem_crop_inputs" in batch:
                    batch["tem_crop_inputs"] = batch["tem_crop_inputs"].float()
                    batch["tem_crop_inputs"] = self.img_normalize(batch["tem_crop_inputs"])
            if is_tuple:
                yield task, batch
            else:
                yield batch
