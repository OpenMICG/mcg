...
from src.datasets.dataset_base import McgBaseDataset, img_collate
...
class McgPretrainSparseDataset(McgBaseDataset):
    """
    ...
    def __init__(self, datalist, tokenizer, img_lmdb_dir, img_db_type, txt_dir,
                ...
                max_img_size=1000, max_txt_len=20, is_train=True):
        super(McgPretrainSparseDataset, self).__init__(
        ...
        if self.is_train:
            self.randaug = TemporalConsistentRandomAugment(N=2, M=5, augs=['Identity',
                                                                           'Contrast',
                                                                           'Brightness',
                                                                           'Sharpness',
                                                                           'ShearX',
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            # LOGGER.info(data_sample)
            video_id = str(data_sample.video_id)  # for others
            txt_len = int(data_sample.txt_len)
            page_dir = data_sample.page_dir
            ...
            # fetch video
            if page_dir != None:
                video_path = os.path.join(self.img_db_dir, page_dir, video_id + self.video_fmt)
            else:
...
class PretrainCollator(object):
    ...
    def collate_batch(self, batch):
        ...
        return dict(
            ...
            type=batch[0]['type'],
            token_type_ids=batch_enc['token_type_ids'])
...
def random_erase(input_img, patch_size, s_l=0.3, s_h=0.5, r_1=0.3, r_2=1/0.3, v_l=0, v_h=255):
    ...
    while True:
        s = np.random.uniform(s_l, s_h) * img_h * img_w     # [0.3, 0.5) * h * w
        r = np.random.uniform(r_1, r_2)     # [3/10, 10/3)
        w = int(np.sqrt(s / r))
        h = int(np.sqrt(s * r))
        left = np.random.randint(0, img_w)      # (0, img_w)
        top = np.random.randint(0, img_h)       # (0, img_h)
