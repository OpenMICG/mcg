...
def load_state_dict_with_pos_embed_resizing(model, loaded_state_dict_or_path,
    ...
    if num_patches + 1 != loaded_state_dict[spatial_embed_key].size(1):
        loaded_state_dict[spatial_embed_key] = resize_spatial_embedding(loaded_state_dict, spatial_embed_key,
                                                                        num_patches)
    else:
    ...
    if temporal_embed_key in loaded_state_dict and num_frames != loaded_state_dict[temporal_embed_key].size(1):
        loaded_state_dict[temporal_embed_key] = resize_temporal_embedding(loaded_state_dict, temporal_embed_key,
                                                                          num_frames)
    else:
        LOGGER.info(
    ...
    LOGGER.info(f"In total {len(diff_keys)}")
    ...
    LOGGER.info(f"In total {len(mismatched_shape_keys)}")
